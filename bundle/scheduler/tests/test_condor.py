...
class TestCondor(TestCase):
    ...
    def test_status_evicted_held_released(self):
        sched = CondorScheduler()
        ...
        with TemporaryDirectory() as td:
            ...
            for event_type in [
                ...
                events = [
                    FakeEvent(htcondor.JobEventType.SUBMIT, cluster=222, log_notes="DAG Node: foo_plot_arg_0"),
                    FakeEvent(event_type),
                ]
                with patch("htcondor.JobEventLog", lambda path: FakeJobEventLog(path, events)):
                    self.assertEqual(sched.status(None, details), (JobStatus.RUNNING, "Job is running"))

    def test_status_terminated_abnormally(self):
        sched = CondorScheduler()
        ...
        with TemporaryDirectory() as td:
            ...
            # A JOB_TERMINATED event that did not terminate normally
            events = [
                FakeEvent(htcondor.JobEventType.SUBMIT, cluster=222, log_notes="DAG Node: foo_plot_arg_0"),
                FakeEvent(
            ...
            with patch("htcondor.JobEventLog", lambda path: FakeJobEventLog(path, events)):
                self.assertEqual(sched.status(None, details), (JobStatus.ERROR, "Job terminated abnormally"))
