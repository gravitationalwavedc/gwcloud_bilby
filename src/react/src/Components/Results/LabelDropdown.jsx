...
const LabelDropdown = ({ data, jobId, onUpdate, modifiable }) => {
    ...
    const labelChoices = data.allLabels.edges.filter(({ node }) => !labels.includes(node.name) && !node.protected);
                    ...
                    <Dropdown>
                        ...
                        <Dropdown.Menu>
                            {labelChoices.map(({ node: { name, description } }) => (
                                <Dropdown.Item key={name} value={name} onClick={() => setLabels([name, ...labels])}>
                                    <h6>{name}</h6>
                                    <p>{description}</p>
                                </Dropdown.Item>
                            ))}
...
const updateJob = (variables, callback) =>
    commitMutation(environment, {
        mutation: graphql`
            mutation LabelDropdownMutation($jobId: ID!, $private: Boolean, $labels: [String]) {
...
export default createFragmentContainer(LabelDropdown, {
    data: graphql`
        fragment LabelDropdown_data on Query @argumentDefinitions(jobId: { type: "ID!" }) {
            bilbyJob(id: $jobId) {
                labels {
                    name
                }
