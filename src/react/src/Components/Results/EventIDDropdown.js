...
const EventIDMenuDisplay = ({ eventId, triggerId, nickname, props }) => (
  <React.Fragment>
    <h5>
      <Highlighter search={props.text}>{`Event ID: ${eventId}`}</Highlighter>
    </h5>
    <Highlighter
      search={props.text}
    >{`Trigger ID: ${triggerId}; Nickname: ${nickname}`}</Highlighter>
...
const EventIDDropdown = ({ eventId, data, setEventId, modifiable }) => {
  ...
  return (
    <Row className="align-items-center">
      ...
      {modifiable && (
        <Button variant="link" className="py-0" onClick={() => setShow(true)}>
          {eventId ? <>Change Event ID</> : "Set Event ID"}
        </Button>
      )}
      <Modal show={show} onHide={() => setShow(false)}>
        <Modal.Header closeButton>
          <Modal.Title>Event ID</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form.Group>
            <Typeahead
              ...
              renderMenuItemChildren={(option, props) => (
                <EventIDMenuDisplay {...option} props={props} />
              )}
            />
          </Form.Group>
        </Modal.Body>
