...
function setup(inputState) {
    const environment = global.queryRendererSetup(
        inputQuery,
        (props) => <SamplerForm data={props.bilbyJob} state={inputState} updateParentState={testEmptyFunction} nextStep={testEmptyFunction}/>
    )
    ...
    return {
        priorChoice: screen.getByLabelText('Default Prior'),
...
describe('Sampler form displays the initial form fields and values correctly:', () => {
    ...
    it('priorChoice', () => {
        expect(fields.priorChoice).toBeInTheDocument()
        expect(within(fields.priorChoice).getByRole('alert')).toHaveTextContent('4s')
    })
