...
describe("editable job name component", () => {
  ...
  it("should call a mutation when changed", async () => {
    ...
    const { container } = render(
      <EditableJobName modifiable={true} value="Testing" jobId={1} />
    );
    userEvent.click(screen.getByText("edit"));
    userEvent.type(screen.getByDisplayValue("Testing"), "-new-value");
    await waitFor(() =>
      userEvent.click(container.getElementsByClassName("save-button")[0])
    );
    await waitFor(() =>
  ...
  it("should display errors correctly", async () => {
    ...
    expect(
      screen.getByText("Remove any spaces or special characters.")
    ).toBeInTheDocument();
  });
  ...
  it("should display graphql mutation errors correctly", async () => {
    ...
    await waitFor(() =>
      environment.mock.resolveMostRecentOperation(() => ({
        ...
        errors: [
          { message: "Duplicate job name found! Bad!" },
          { message: "So bad. Do a stop!" },
        ],
      }))
    );
    expect(
      screen.getByText(/Duplicate job name found! Bad!/)
    ).toBeInTheDocument();
  ...
  it("should clear errors when the mutation is successful", async () => {
    ...
    expect(
      screen.queryByText(/Duplicate job name found! Bad!/)
