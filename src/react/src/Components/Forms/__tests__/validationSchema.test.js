...
describe("validation schema", () => {
  it("should check that a detector has been selected", async () => {
    ...
    expect(() => validationSchema.validateSync()).toThrow(
  ...
  it("should check that the channel and detector match", async () => {
    ...
    expect(() =>
      validationSchema.validateSync({ hanford: true, hanfordChannel: "GWOSC" })
    ...
    expect(() =>
      validationSchema.validateSync({ virgo: true, virgoChannel: "GWOSC" })
    ).toThrow("dlogz is a required field");
