...
class BilbyJobNode(DjangoObjectType):
    ...
    def resolve_params(self, info):
        try:
            ...
            logger.warning("Failed to generate parameter output for job %s: %s", self.id, type(e).__name__)
            return None
...
class UpdateBilbyJobMutation(relay.ClientIDMutation):
    ...
    def mutate_and_get_payload(cls, _root, info, **kwargs):
        ...
        try:
            ...
            raise GraphQLError(f"Job '{job_model_id}' not found.")
        except PermissionError:
            raise GraphQLError("You must own the job to change it!")
