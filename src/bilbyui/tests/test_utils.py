...
def construct_ini_string_from_dict(config_dict):
    config_line_list = [f"{key}={value}" for key, value in config_dict.items()]
...
def create_test_upload_data(
    ...
    job_label,
    include_result=True,
    include_results_page=True,
    include_data=True,
    multiple_ini_files=False,
    no_ini_file=False,
...
def get_file_download_tokens(response):
    ...
    download_tokens = [
        f["downloadToken"] for f in filter(lambda x: not x["isDir"], response.data["bilbyResultFiles"]["files"])
    ]
...
def get_files(response):
    ...
    files = [f for f in filter(lambda x: not x["isDir"], response.data["bilbyResultFiles"]["files"])]
    return files
...
def generate_elastic_doc(job, user):
    doc = {
        ...
        "eventId": None,
        "ini": {kv.key: json.loads(kv.value) for kv in job.inikeyvalue_set.filter(processed=False)},
        "params": {kv.key: json.loads(kv.value) for kv in job.inikeyvalue_set.filter(processed=True)},
