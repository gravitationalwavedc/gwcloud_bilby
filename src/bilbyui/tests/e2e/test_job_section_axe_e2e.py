...
class TestJobSectionAxeScan(JobSectionShellBase):
    def assert_no_accessible_name_or_label_violations(self, violations):
        blocking = [violation for violation in violations if violation.get("id") in ACCESSIBLE_NAME_AND_LABEL_RULES]
        detail = "\n".join(
    ...
    async def test_no_accessible_name_or_label_axe_violations(self):
        ...
        await self.page.wait_for_selector(f"{AXE_SCOPE_SELECTOR} #job-section-heading")
        ...
        await self.page.locator('input[name="name"], button[aria-label="Save name"]').first.wait_for(state="visible")
        violations = await run_axe(self.page, AXE_SCOPE_SELECTOR)
        self.assert_no_accessible_name_or_label_violations(violations)

        await self.page.locator('button[aria-label="Edit description"]').click()
        await self.page.locator('textarea[name="description"], button[aria-label="Save description"]').first.wait_for(
            state="visible"
        )
        violations = await run_axe(self.page, AXE_SCOPE_SELECTOR)
        self.assert_no_accessible_name_or_label_violations(violations)
