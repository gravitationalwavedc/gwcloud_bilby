...
def _make_file_entry(real_file_name, is_dir, job_dir):
    ...
    with contextlib.suppress(OSError, ValueError):
        # OSError happens when trying to stat a broken symlink or a symlink
        # loop (e.g. ELOOP), PermissionError when the file is unreadable,
        # and ValueError when the path escapes the job directory.
        return {
...
def request_file_list(job, path, recursive, user_id=None):
    ...
    try:
        ...
        logger.warning("Error getting job file list: %s", e)
        return False, "Error getting job file list"
