...
urlpatterns = [
    ...
    path(
        "job-results/<int:job_id>/files/<str:token>/download/",
        views.file_download_redirect,
        name="file_download",
    ),
    path(
        "job-results/<int:job_id>/parameters/",
        views.view_job_parameters_partial,
        name="view_job_parameters",
    ),
    path(
        "job-results/<int:job_id>/results/",
        views.view_job_results_partial,
        name="view_job_results",
    ),
    path(
        "job-results/<int:job_id>/field/<str:field>/",
        views.view_job_field_partial,
        name="view_job_field_partial",
    ),
    path("job-results/<int:job_id>/edit/name/", views.edit_job_name, name="edit_job_name"),
    path(
        "job-results/<int:job_id>/edit/description/",
        views.edit_job_description,
        name="edit_job_description",
    ),
    path(
        "job-results/<int:job_id>/edit/privacy/",
        views.edit_job_privacy,
        name="edit_job_privacy",
    ),
    path(
        "job-results/<int:job_id>/edit/labels/",
        views.edit_job_labels,
        name="edit_job_labels",
    ),
    path("event-ids/", views.event_id_search, name="event_id_search"),
    path(
        "job-results/<int:job_id>/event-id-modal/",
        views.event_id_modal,
        name="event_id_modal",
    ),
    path(
        "job-results/<int:job_id>/edit/event-id/",
        views.edit_job_event_id,
        name="edit_job_event_id",
    ),
    path("job-results/<int:job_id>/", views.view_job_view, name="view_job"),
