...
sudo docker run --env-file .env --network=host --mount type=bind,src="$HOST_DB_PATH",target="$DB_PATH" --mount type=bind,src="./gwosc_ingest.log",target="/app/gwosc_ingest.log" gwosc_ingest
